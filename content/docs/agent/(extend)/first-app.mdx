...
company.uppercase Plugin
  ...
  packages a trusted Process implementation
...
You need Rust 1.94 or newer, Git, and either distribution of the Lenso CLI:
...
The default Rust project has one editable `src/lib.rs` and produces a trusted
native Process implementation of the Plugin Contract.
...
The result contains `HELLO LENSO`. With the default Process scaffold, `auto`
selects the Process implementation.
...
`dist/company.uppercase-0.1.0.lenso-plugin`. That archive contains the Process
implementation and one shared Contract.
