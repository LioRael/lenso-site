...
host and Lenso Console can see: routes, admin surfaces, runtime functions,
...
use lenso::{
    ...
    ModuleHttpRoute, ModuleManifest, CONSOLE_BRIDGE_PROTOCOL,
};
...
| Field | Purpose |
| --- | --- |
| `module_id` | Canonical Module id, such as `lenso/auth` or `acme/billing`. The builder normalizes short first-party names. |
...
| `console` | Declarative or isolated Console surfaces. Executable UI is bound by digest in the same Module Release, never by a same-origin package export. |
