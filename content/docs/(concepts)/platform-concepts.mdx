...
| Provider Service | `lenso.service.v1` | Provides Modules from a separate Rust or TypeScript process while the Host owns runtime coordination. |
...
2. **Run locally** with `lenso system dev` and the Local Control Adapter.
3. **Connect** signed local Service enrollment, the exact `lenso.system.v2` topology,
   and its Management Binding to the separately installed Console Service.
...
- Provider `lenso.service.v1` is Host-owned and supports Rust and TypeScript.
