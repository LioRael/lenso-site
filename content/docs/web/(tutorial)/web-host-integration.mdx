...
lowering, and linked factory. Keep the implementation from the previous lesson
in the crate root at `src/lib.rs`; there is no separate `src/http.rs` module to
declare:

```rust title="src/lib.rs"
// GreetingsHttp and its #[endpoint] impl remain here.
```
