...
OpenAPI support is an ordinary opt-in Module, not an Ingress setting. Endpoint
...
The macro validates the JSON and rejects an author-supplied `operationId` that
could drift from the stable route ID. Method and path always come from the
route declaration. Routes without OpenAPI metadata remain valid; the document
assembler gives them a minimal fallback response when they are included.
...
Module does not discover providers globally or infer a public server URL from
