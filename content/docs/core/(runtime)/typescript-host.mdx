...
description: Prepare real runtime inputs, verify the Host lifecycle, and understand the published 0.16.1 Bun Plugin limits.
---
...
2026-09-07. They describe the published `@lenso/cli@0.16.1` cohort, not current
unpublished source candidates. It was not a working end-to-end Bun Plugin Host
quickstart. First verify the real lifecycle with an empty Host, then read the
cohort limit below.
...
complete Host runtime. Current Rust ownership is consolidated in `LioRael/lenso`.
To reproduce this historical cohort, build each binary from the imported source
revision and its original lockfile rather than relying on a global CLI:

```sh
git clone https://github.com/LioRael/lenso.git runtime-source
...
git clone https://github.com/LioRael/lenso.git owner-source
...
## Add a real Bun Plugin: published 0.16.1 cohort limit
...
`lenso.bun-authoring@2`, while that published builder's V2 admission uses
...
See the [Host declaration reference](https://github.com/LioRael/lenso/blob/main/crates/lenso-cli/docs/typescript-host-authoring.md#admit-extensions-and-constrain-configuration)
