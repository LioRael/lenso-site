...
The updated local skills include source Apps, optional conventions, App Console and automatic Capability generation. See [local DX](/docs/core/engine-dx) and [file conventions](/docs/core/file-conventions). Remote installation below does not include unpublished local changes; use the updated local skill pack until release.
