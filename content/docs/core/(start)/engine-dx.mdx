...
Engine remains independently consumable: `lenso-engine 0.1.1` and the optional App,
Runtime, Authoring, Markdown, Worker and Host packages at `0.1.0` are published
on crates.io. Their current Rust source is maintained in the consolidated
`LioRael/lenso` repository. The generic Engine can be consumed directly as a
library. CLI and Console development kits have separate release paths. Before using the App
...
Engine is an independently embeddable authoring layer in the consolidated
`LioRael/lenso` repository. Rust packages live under `crates/`; the CLI consumes
the libraries.
