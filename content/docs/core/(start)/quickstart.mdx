...
To create a complete App from local sources, use [Local App development and Engine](/docs/core/engine-dx). That path requires the matching local build; the standalone Plugin quickstart below retains its published-CLI prerequisites.
...
The default Rust scaffold produces a trusted Process implementation. Bun uses
the generated `@lenso/bun` runtime lowering.

## Prerequisites

- Rust stable for the default Rust scaffold;
...
the generic CLI. Once you have that matching pair, follow
[Change your first App](/docs/core/first-app-change) to initialize a workspace,
add the local Bundle, inspect the derived App, run it, and remove the change.
Catalog installation is an optional path and only applies to a Release that
the selected catalog actually returns.

Run `lenso doctor` when setup or resolution fails. It emits human-readable
checks by default and stable JSON with `--json`.

Continue with [Change your first App](/docs/core/first-app-change),
[Plugin authoring](/docs/core/plugin-authoring), or
[Bun Plugin authoring](/docs/core/bun-plugin-authoring).
