...
In the matching source-App workflow, `lenso app contract new example.text` or `--source rust` scaffolds a contract, and `app build` / `app dev` synchronize selected projections. See [local development](/docs/core/engine-dx). The role, contract and compatibility rules below still apply.
...
See the maintained [Request fixture](https://github.com/LioRael/lenso-protocols/tree/main/crates/lenso-contract-codegen/tests/fixtures/wit)
for a complete request contract. It uses `example.greeting@1` with an `echo`
operation and a WIT projection, so it demonstrates the generated layout rather
than providing a drop-in `greet` fixture. The maintained
[Stream fixture](https://github.com/LioRael/lenso-protocols/tree/main/crates/lenso-contract-codegen/tests/fixtures/stream)
and [Event fixture](https://github.com/LioRael/lenso-protocols/tree/main/crates/lenso-contract-codegen/tests/fixtures/event)
are also available.
