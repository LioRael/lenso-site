...
login, Runtime Console access, or both.
...
`auth.users` is the shared authentication anchor. Business routes and Runtime
Console both resolve to the same `ActorContext::User { user_id, scopes }`.
...
Runtime Console admin access is explicit. A normal business user does not become
a Console admin unless you add `console.admin` to that auth user through runtime
config:
...
Grant the first production admin with `lenso console bootstrap-admin`, then edit
`auth.console_admin_user_scopes` from Console Config for later admins.
Development bearer tokens are local-only.
