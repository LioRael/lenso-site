...
`auth-password` and `auth-device` depend on `auth`. Lenso Console exposes
...
| File | Purpose |
| --- | --- |
| `.env` | records the CLI-managed Service provider entry and declared env values |
...
Lenso Console's Available Modules view reads, in order:
...
## Lenso Console workflow

Lenso Console creates a reviewed Module change plan, records approval and
submission evidence, and observes the owning target's terminal receipt. The
browser does not write Host-local install files or install npm packages.
