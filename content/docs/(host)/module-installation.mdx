...
service provider, a bundled host module, or a future sandbox. `lenso service
install` is lower level: it connects a provider process, but it is not the
...
Available Modules uses discovery data only. It does not install a module until
the operator runs an install action.
...
The built-in catalog is the default discovery source for fresh hosts. It is not
a hidden install database.
