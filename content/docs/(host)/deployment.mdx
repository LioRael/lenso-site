...
history. Lenso Console reads that Host-local evidence; it does not need
...
Install independently deployed providers through an approved Service manifest
and verify them before routing production traffic:
...
- Service providers use Workload Identity or provider credentials, never caller tokens.
