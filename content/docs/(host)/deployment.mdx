---
title: Deployment
description: Deploy the API, worker, migrations, Postgres, and an independent Lenso Console without overfitting to microservices.
---
...
The host archive does not embed Console web assets. Console uses its own reviewed
Service Release, Store, Migration Workload, API, Worker, and Shell.

## Deployment checklist

- `APP_ENV` and `LENSO_COMPOSITION_PROFILE` are explicit.
- Postgres migrations run before API and worker.
- `lenso console doctor` verifies the independent Console installation.
