...
Services are separate processes with their own Stores. Installing a Service can
record provider and lifecycle state, but the Host migration runner never applies
that Service's private migrations.
...
- Make SQL idempotent only when it is naturally safe; the runner already tracks
  applied names.
- Keep Service Store migrations in the owning Service's migration Workload and
  deployment process.
