...
Provider implementations export the same Module contract over
`lenso.service.v1`. Neither delivery mode changes the Module name or operation
IDs.
...
6. Compose a new `lenso.app.json` revision and run it through
   `lenso system dev`.
