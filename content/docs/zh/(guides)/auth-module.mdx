---
title: Auth Module
description: 认证 Adapter 选出的凭据，传递签名 ActorAssertion，并在目标 Module 内完成授权。
---
...
第一个具体 Provider 是 PostgreSQL API Token Module。Schema 初始化和 Token
...
能力。它们应成为同一 Capability 后面的独立 Ingress 或 Provider Module，而不是
新的 Kernel 行为。
...
| API Token Auth Module | PostgreSQL Provider、Operator Workflow、撤销 | 源码已实现；Crate 暂时保持私有 |
