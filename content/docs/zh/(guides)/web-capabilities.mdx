...
OpenAPI 支持是普通的 opt-in Module，不是 Ingress 设置。Endpoint 作者可以给 route
...
OpenAPI Module 把它纳入文档时，assembler 才会为它补上最小 fallback response。
...
生成的 OpenAPI 3.1 文档只包含这个显式绑定的子集。Module 不会全局发现 provider，
