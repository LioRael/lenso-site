...
macro 会校验 JSON，并拒绝作者另行提供可能与稳定 route ID 漂移的 `operationId`。
method 与 path 始终来自 route declaration。没有 OpenAPI metadata 的 route 仍然
有效；被纳入文档时，assembler 会为它补上最小 fallback response。
