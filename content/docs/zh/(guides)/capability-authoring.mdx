...
新建 Request Capability 与 Bun Provider 时，`lenso module create NAME
--runtime bun` 会一起生成 Descriptor、Schema、两份 Binding 和类型化 Provider。
...
生成的 TypeScript 会直接引用 `@lenso/contract-runtime`。Bun Module Scaffold
会通过 `@lenso/bun-module` 安装它；独立 Contract Package 可以直接添加：
