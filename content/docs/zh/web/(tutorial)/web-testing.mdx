...
要求得到 `201`、JSON Content Type 与稳定 ID。通过 Search Route 查询刚创建的 Greeting：
...
要求得到 `200`，并在一行结果中看到相同的 ID 与 Message。
...
| 任一路由的无效 JSON | Endpoint Extractor | Handler 运行前返回 `400`。 |
...
两条请求都必须得到 Ingress 的 Not Found 响应。这证明后端行为属于可移除
Plugin，而不是隐藏的 Host 代码。
...
`POST /greetings` 与 `QUERY /greetings/search` 的成功与失败矩阵通过真实 Socket，
重复 Route 阻止 Readiness，并且移除会从下一 Generation 删除两条 Route 时，后端才算完成。
