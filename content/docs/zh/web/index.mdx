...
公共 CLI 支持 `lenso plugin new <plugin-id> --web`，用于创建 Linked Native Rust
Web Plugin 脚手架。它的 `lenso plugin dev` 命令可以启动临时 Development Host
和真实 Listener，完成本地证明。永久部署仍需要产品 Host 链接 Plugin，并发布下文
描述的 Host Catalog 关系。
...
遇到缺失项时，在那里停下并与 Host Owner 解决。当前 Web Capability 与 Transport
Crate 由 `LioRael/lenso` Monorepo 持有；已发布 Cohort 使用
`lenso-capability-http-endpoint` 0.2.3 和 `lenso-web-ingress` 0.3.3。其他 Host
应使用自身 Lock 选择的准确版本。

## 完成条件

`POST /greetings` 与 `QUERY /greetings/search` 通过真实 Ingress 运行，成功与失败保持可区分，重复 Route 阻止
