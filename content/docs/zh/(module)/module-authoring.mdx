...
Linked 实现把 Route Contribution 接入 `LinkedBinding`。Provider 实现通过
`lenso.service.v1` 导出同一 Module Contract。两种交付模式都不会改变 Module
名称或 Operation ID。
...
6. 组合新的 `lenso.app.json` 修订，并通过 `lenso system dev` 运行。
