...
Manifest 告诉 Host 哪些能力应当出现在 Lenso Console 中，但它不会替代
...
只有 Module 确实需要自己的运维页面时才加入隔离 Console UI 制品：

```sh
lenso module create billing --with-console-ui
```

一般的数据、Action 和运行时证据已经会出现在 独立运行的 Lenso Console 中。
...
5. 打开 配置的 Console Service URL，确认能力和证据可见。
...
- 配置的 Console Service URL 能显示运维人员需要的模块证据
