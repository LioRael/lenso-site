...
`LioRael/lenso` 中的 ADR 0030–0077 是当前架构规范。可以从以下决策组开始：
...
| 把本地 Source Discovery 作为生成的 Host 输入 | 0075 |
| 区分 Execution Environment 与 Infrastructure Implementation | 0076 |
| Rust Main-chain 仓库合并 | 0077 |

ADR 接受不代表每个 Profile 都已实现或发布；准确发布证据以 Package Version 与
Lockfile 为准。JavaScript SDK Surface 见
[Bun 交互支持](https://github.com/LioRael/lenso-js/tree/main/packages/lenso-bun-plugin)。
