...
description: 准备真实运行文件，验证 Host 生命周期，并理解已发布 0.16.1 Cohort 的 Bun Plugin 限制。
---
...
2026-09-07 在 macOS ARM64 上从新目录验证的结果如下。它描述的是已发布的
`@lenso/cli@0.16.1` Cohort，不代表当前未发布的源码候选。它还不能作为带 Bun Plugin
的一键启动教程；先用空 Host 验证真实生命周期，再阅读下面的 Cohort 限制。
...
npm CLI 自带当前平台的 Resolver 和生成控制库，不自带完整 Host 运行环境。当前 Rust
所有权已经合并到 `LioRael/lenso`。要复现这个历史 Cohort，应分别使用合并进来的源码
Revision 及其原始 Lockfile 构建两个二进制，不要依赖同名的全局 CLI：
...
## 添加真实 Bun Plugin：已发布 0.16.1 Cohort 的限制
...
该 Bundle 声明 `lenso.bun-authoring@2`，而该已发布 Host Builder 的 V2 准入使用
...
关键字见[Host 声明参考](https://github.com/LioRael/lenso/blob/main/crates/lenso-cli/docs/typescript-host-authoring.md#admit-extensions-and-constrain-configuration)。
