...
在配套源码 App 工作流中，`lenso app contract new example.text` 和 `--source rust` 可创建合约，`app build` / `app dev` 自动同步选中投影；见[本地开发](/docs/zh/core/engine-dx)。下文仍定义角色、合约与兼容性规则。
...
可直接参考持续维护的 [Request fixture](https://github.com/LioRael/lenso-protocols/tree/main/crates/lenso-contract-codegen/tests/fixtures/wit)。
它使用 `example.greeting@1` 与 `echo` Operation，并包含 WIT Projection，因此它展示
的是生成物布局，而不是可以直接替换本页 `greet` 的同名 fixture。持续维护的
[Stream fixture](https://github.com/LioRael/lenso-protocols/tree/main/crates/lenso-contract-codegen/tests/fixtures/stream)
和 [Event fixture](https://github.com/LioRael/lenso-protocols/tree/main/crates/lenso-contract-codegen/tests/fixtures/event)
也可参考。
