...
验证其 Closure。`dev` 直接调用同一份 Provider，作者无需实现测试 Wire；上面请求
的响应是 `HELLO`。Watch Mode 在一次构建失败后仍保持运行。
...
类型化 SDK 见 [`@lenso/bun`](https://www.npmjs.com/package/@lenso/bun) 与
[`lenso-js` 源码](https://github.com/LioRael/lenso-js/tree/main/packages/lenso-bun-plugin)，
进程边界见 [`lenso` Adapter](https://github.com/LioRael/lenso/tree/main/crates/lenso-bun-adapter)。
...
具体生成接口见 [Bun SDK](https://github.com/LioRael/lenso-js/tree/main/packages/lenso-bun-plugin)。
