...
主机和 Lenso Console可以看到：路由、管理界面、运行时功能、
运行时计划、事件、生命周期工作、控制台包、依赖项和
能力。

可执行的行为并不存在于Manifest中。链接模块附加
通过 `LinkedBinding` 的行为；Service通过 HTTP 或
...
| `console` |Lenso Console前端界面由受信任的包或托管包支持。 |
...
主机通过模块元数据和 Lenso Console公开 `manifest_lints`
将它们分为几类：
...
[Lenso Console](/docs/zh/runtime-console) 验证主机可见结果。
