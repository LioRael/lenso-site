...
| --- | --- | --- | --- | --- |
| Provider tier | `lenso.service.v1` | Host | Rust 与 TypeScript | Service 进程提供 Module，Host 继续持有 Policy、Queue、Retry、Outbox 分发与 Runtime Story。 |
...
该层级支持 Rust 和 `@lenso/service-kit`。TypeScript Kit 生成并提供
`lenso.service.v1` Manifest、Module Release、HTTP Handler 和受支持的 Provider
Transport Surface。
