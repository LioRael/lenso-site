---
title: 配置
description: 配置Lenso主机、模块源、Lenso Console 路径、遥测和模块本地值。
---
...
| `CORS_ALLOWED_ORIGINS` |逗号分隔的 Lenso Console开发起源 |
| `RUST_LOG` |日志级别/过滤器 |
| `LOG_FORMAT` | `compact` 或 `json` |
| `OTEL_EXPORTER_OTLP_ENDPOINT` |可选的 OpenTelemetry 收集器端点 |

对于运行状况探测、结构化日志和 Lenso Console 运行时视图，请阅读
...
值运算符的配置描述符应通过 Lenso Console进行编辑。
