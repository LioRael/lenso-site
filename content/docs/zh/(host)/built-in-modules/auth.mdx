...
Runtime Console扩展注册表。安装后重启API和worker
或更新模块。
...
Runtime Console管理员访问是明确的。普通企业用户不会成为
控制台管理员，除非您通过运行时将 `console.admin` 添加到该身份验证用户
配置：
...
授予第一个生产管理员 `lenso console bootstrap-admin`，然后编辑
来自控制台配置的 `auth.console_admin_user_scopes` 供以后的管理员使用。
开发Bearer Token仅限本地使用。
