...
| --- | --- | --- | --- |
| [可移植 Rust Plugin](/docs/zh/plugin-authoring) | 添加一个需要打包、安装的 Agent Tool | Wasm、可信 Process，或两者 | `lenso plugin new` 当前只脚手架化 Agent Tool Provider Capability |
| [Linked Rust Plugin](/docs/zh/linked-rust-plugin) | 你拥有产品 Host，需要深层、有状态或提供多个 Capability 的实现 | 链接进 Host 的原生 Rust | Host 二进制必须包含生成的 factory |
| [Bun Plugin](/docs/zh/bun-plugin-authoring) | 希望用 TypeScript 实现强类型 Request provider | 可信 Bun 子进程 | 公共 SDK 当前只开放 Request Capability |
...
接下来可以跟随[实现第一个 App 行为](/docs/zh/first-app)完成一次真实安装，或从
表格进入对应路径的教程。
