...
| 步骤 | 文档 | 可观察结果 |
| --- | --- | --- |
| 1 | [编写 Endpoint Plugin](/docs/zh/web-endpoint-plugin) | 两条稳定 Route Description 与直接 Handler 测试通过。 |
| 2 | [连接 Host 与 Ingress](/docs/zh/web-host-integration) | Resolved App 显示 Endpoint Instance 已绑定到 Ingress。 |
| 3 | [证明 HTTP 后端](/docs/zh/web-testing) | 成功、失败、冲突与移除路径都通过真实 Socket。 |

修改 Middleware、Limit、Cancellation 或出站 HTTP 时，阅读
[Web Capabilities](/docs/zh/web-capabilities)。Route 需要认证 Evidence 与传播
Assertion 时，阅读 [Auth Plugin](/docs/zh/auth-plugin)。
...
从 [编写 Endpoint Plugin](/docs/zh/web-endpoint-plugin) 开始。
