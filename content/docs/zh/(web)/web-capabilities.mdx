...
第一次构建后端时，请先按照[开发 Web 后端](/docs/zh/web-backend)完成 Host 前置条件、
Endpoint Crate、Linked Registration、Ingress 配置、真实 Socket 调用与移除。完成后
再回到本页查阅 HTTP Contract、Auth、OpenAPI、Middleware 与 Egress 的详细参考。

```mermaid
flowchart LR
  H[HTTP client] --> I[Web Ingress]
  I --> E[HTTP Endpoint providers]
  I --> O[可选 OpenAPI document]
  O -. 显式绑定的 descriptions .-> E
...
Contribution Plugin 负责组合页面和生成的浏览器 client，并不拥有后端 HTTP route。
...
`#[put]`、`#[patch]`、`#[delete]`、`#[head]`、`#[options]` 与 `#[query]`；每个属性依次声明
稳定 route ID 和 path。旧的 `http_endpoint!` 静态表仍可用于生成代码或兼容已有
Plugin。

`#[query]` 表示 HTTP QUERY 方法：它适合安全、幂等、但需要结构化请求体的查询。
不要把它与 URL Query String 混淆；URL 参数使用 `QueryParams<T>`：
...
QUERY 请求携带内容时必须声明正确的 `Content-Type`；`Json<T>` 会在 Handler 前执行
这一检查。旧名称 `Query<T>` 暂时保留用于迁移，但新代码应使用 `QueryParams<T>`。
...
OpenAPI 支持是普通的 opt-in Plugin，不是 Ingress 设置。Endpoint 作者可以给 route
附加 OpenAPI 3.1 Operation Object，同时继续由 route declaration 权威定义 method、
path 与稳定的 `operationId`：
...
JSON-like DSL 的普通 identifier key 不需要引号；`"type"`、
`"application/json"`、`"$ref"` 等 key 仍使用引号。原有 raw JSON string 写法继续
兼容，`http_endpoint!` route table 也可以使用
`openapi = openapi_operation!({ ... })`。macro 会在编译期校验 DSL，并拒绝作者另行
提供可能与稳定 route ID 漂移的 `operationId`。method 与 path 始终来自 route
declaration。

注解本身仍然是可选的。没有 OpenAPI metadata 的 route 依旧有效；只有显式选择的
OpenAPI Plugin 把它纳入文档时，assembler 才会为它补上最小 fallback response。

要发布文档，App Composition 必须显式完成三个选择：

1. 把已发布的 `lenso-openapi` crate 选为一个 `lenso.openapi` Instance。
2. 把需要写入文档的 Endpoint provider 绑定到该 Instance 的 `many`
   `lenso.http.endpoint@1` requirement。
3. 把 OpenAPI Instance 自己提供的 HTTP Endpoint 绑定到 Web Ingress。

生成的 OpenAPI 3.1 文档只包含这个显式绑定的子集。Plugin 不会全局发现 provider，
也不会从 listener 推断公开 server URL。不可变 configuration 控制文档 metadata 与
静态 route：
...
这里有意不提供 `enabled` 字段。删除 `lenso.openapi` Instance 及其 binding 就会移除
`/openapi.json`，不会改动业务 Endpoint 或 Web Ingress。Swagger UI、Redoc、页面、
asset 与 navigation 仍属于 App 或 Console 持有的 UI 关注点。
...
Plugin 仍必须校验 assertion 的 proof、issuer、有效期和 operation audience，然后再投影
为自己的 typed actor。Provider 与目标侧的完整代码见 [Auth Plugin](/docs/zh/auth-plugin/)。

extractor 按 handler 参数顺序运行。它可以等待 provider 上保存的 client、为后续
extractor 和 handler 增强 context、返回明确响应，或保留 Endpoint failure。内置
extractor 包括 `Path<T>`、`QueryParams<T>`、`Json<T>` 与 `RequestId`。无效 path、query
...
直接授予角色、tenant access 或 resource ownership；这些决定仍属于目标 Plugin。

`handle_authenticated` 是 Endpoint 自己的应用代码：它携带上述 assertion context
调用 `self.orders`，用 `response::json` 序列化成功结果，并把目标 Plugin 的授权拒绝
...
Ingress 拥有 transport error；Auth 拥有 evidence authentication；目标业务 Plugin
...
Ingress 与 Egress provider 是 linked Rust Plugin；Bun Plugin 仍需通过 host 支持的
